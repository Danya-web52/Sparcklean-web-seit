// SparkClean site behavior
  ...
  // ---- gallery placeholders (swap ph-slot backgrounds for real <img> tags later) ----
  const galleryItems = [
    {label:'Kitchen counters', cat:'kitchen'},
    {label:'Kitchen appliances', cat:'kitchen'},
    {label:'Shower & tub', cat:'bathroom'},
    {label:'Toilet sanitizing', cat:'bathroom'},
    {label:'Cabinet detail', cat:'detail'},
    {label:'Baseboards', cat:'detail'},
    {label:'Window frames', cat:'windows'},
    {label:'Window glass', cat:'windows'},
  ];
  const placeholderIcon = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="5" width="18" height="14" rx="2"/><circle cx="9" cy="11" r="2"/><path d="m21 15-4.5-4.5L9 18"/></svg>`;
  function renderGallery(filter){
    const grid = document.getElementById('galleryGrid');
    const items = filter==='all' ? galleryItems : galleryItems.filter(i=>i.cat===filter);
    grid.innerHTML = items.map((i,idx)=>`
      <div class="gallery-card reveal reveal-${(idx%4)+1}">
        <div class="gallery-photos">
          <div class="ph-slot">${placeholderIcon}<span>BEFORE PHOTO</span></div>
          <div class="ph-slot after-slot">${placeholderIcon}<span>AFTER PHOTO</span></div>
        </div>
        <div class="gallery-meta">
          <strong>${i.label}</strong>
          <span class="gallery-tag">${i.cat}</span>
        </div>
      </div>
    `).join('');
    observeReveals();
  ...
  // contact form (front-end only demo — wire to email/CRM as needed)
